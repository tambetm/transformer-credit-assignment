...
Algorithm:
1. Collect episodes
2. Compute episode return G
3. Normalize: A_episode = (G - mu_EMA) / sigma_EMA
4. Apply A_episode uniformly to all timesteps
5. Policy gradient: nabla J = sum_t A_episode * nabla log pi(a_t|s_t)
...
class ReinforceEMA:
    ...
    def update(self, buffer: TrajectoryBuffer):
        ...
        for episode in buffer.episodes:
            ...
            # Update EMA and compute advantage
            advantage = self.ema.normalize(G)
            self.ema.update(G)

            tensors = episode.to_tensors(self.device)

            # Apply uniform advantage to all timesteps
